...
require_relative 'WeatherData_types'


class WeatherDataSDK
  ...
  def initialize(options = {})
    ...
    utility = WeatherDataUtility.new
    @_utility = utility
    ...
    feature_opts = WeatherDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WeatherDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WeatherDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WeatherDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WeatherDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WeatherDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WeatherDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WeatherDataError.new(
        "#{op}_allow",
        "WeatherDataSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WeatherDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WeatherDataError.new(
        "graphql_error", "WeatherDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WeatherDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
