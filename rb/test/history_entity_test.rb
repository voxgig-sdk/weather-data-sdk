...
class HistoryEntityTest < Minitest::Test
  def test_create_instance
    testsdk = WeatherDataSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = WeatherDataSDK.test(seed, nil)
    seen = base.History(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = WeatherDataConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = WeatherDataSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WEATHER_DATA_TEST_HISTORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def history_basic_setup(extra)
  ...
  entid_env_raw = ENV["WEATHER_DATA_TEST_HISTORY_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["WEATHER_DATA_TEST_HISTORY_ENTID"])
  ...
  if env["WEATHER_DATA_TEST_LIVE"] == "TRUE"
    ...
    client = WeatherDataSDK.new(Helpers.to_map(merged_opts))
  end
  ...
  live = env["WEATHER_DATA_TEST_LIVE"] == "TRUE"
  {
    ...
    env: env,
    explain: env["WEATHER_DATA_TEST_EXPLAIN"] == "TRUE",
    live: live,
