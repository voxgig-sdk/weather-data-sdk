...
class WeatherEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WEATHERDATA_TEST_WEATHER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def weather_basic_setup(extra)
  ...
  entid_env_raw = ENV["WEATHERDATA_TEST_WEATHER_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "WEATHERDATA_TEST_WEATHER_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["WEATHERDATA_TEST_WEATHER_ENTID"])
