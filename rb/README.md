# WeatherData Ruby SDK



The Ruby SDK for the WeatherData API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.History` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = WeatherDataSDK.test(testopts, sdkopts)
```
...
raise a `WeatherDataError` (a `StandardError` subclass), so wrap
...
```
rb/
├── WeatherData_sdk.rb       -- Main SDK module
...
The main module (`WeatherData_sdk`) exports the SDK class
