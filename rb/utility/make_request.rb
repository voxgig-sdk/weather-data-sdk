...
module WeatherDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WeatherDataResponse.new({})
    ...
    if fetch_err
      ...
      response = WeatherDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeatherDataResponse.new(fetched)
    else
