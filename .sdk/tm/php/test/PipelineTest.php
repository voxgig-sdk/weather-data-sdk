...
{
    private static function utility(): WeatherDataUtility
    {
        return new WeatherDataUtility();
    }
    ...
    private static function ctx(array $over = []): WeatherDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WeatherDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WeatherDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WeatherDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WeatherDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WeatherDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WeatherDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WeatherDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WeatherDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WeatherDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WeatherDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new WeatherDataResponse([]),
            'result' => new WeatherDataResult([]),
        ]);
        [, $err] = WeatherDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WeatherDataSpec([]),
            'response' => null,
            'result' => new WeatherDataResult([]),
        ]);
        [, $err] = WeatherDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WeatherDataSpec(['step' => 's']),
            'response' => new WeatherDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WeatherDataResult([]),
        ]);
        [, $err] = WeatherDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherDataSpec(['step' => 's']),
            'response' => new WeatherDataResponse(self::resp(200, ['v' => 1])),
            'result' => new WeatherDataResult([]),
        ]);
        [, $err] = WeatherDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherDataSpec(['step' => 's']),
            'response' => new WeatherDataResponse(self::resp(200, ['v' => 2])),
            'result' => new WeatherDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WeatherDataMakeResponse::call($ctx);
    ...
    {
        $preset = new WeatherDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WeatherDataSpec([]),
            'response' => new WeatherDataResponse([]),
            'result' => new WeatherDataResult([]),
        ...
        [$got, $err] = WeatherDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WeatherDataResult([])]);
        [, $err] = WeatherDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WeatherDataSpec([]), 'result' => null]);
        [, $err] = WeatherDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherDataSpec(['step' => 's']),
            'result' => new WeatherDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WeatherDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherDataSpec(['step' => 's']),
            'result' => new WeatherDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WeatherDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WeatherDataSpec([]),
            'result' => new WeatherDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WeatherDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WeatherDataError('boom', 'boom');
        $utility->fetcher = function (WeatherDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WeatherDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WeatherDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WeatherDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WeatherDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        WeatherDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WeatherDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WeatherDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WeatherDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WeatherDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WeatherDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WeatherDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WeatherDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WeatherDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WeatherDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WeatherDataResult(['ok' => false])]);
        $this->expectException(WeatherDataError::class);
        WeatherDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WeatherDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WeatherDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WeatherDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WeatherDataBaseFeature();
        $b = new WeatherDataBaseFeature();
        WeatherDataFeatureAdd::call($ctx, $a);
        WeatherDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WeatherDataBaseFeature
    {
        $f = new WeatherDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WeatherDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WeatherDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WeatherDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        WeatherDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WeatherDataResponse(['headers' => null]),
            'result' => new WeatherDataResult([]),
        ]);
        WeatherDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        WeatherDataResultBody::call($ctx);
