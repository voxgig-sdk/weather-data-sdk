...
import (
	...
	omni "github.com/voxgig-sdk/weather-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WeatherDataError); is {
				out := map[string]any{
					"name":    "WeatherDataError",
...
func omniCtx(arg any, client *sdk.WeatherDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WeatherDataSDK); is {
			client = live
