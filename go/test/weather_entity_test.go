...
func TestWeatherEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WEATHER_DATA_TEST_WEATHER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		weatherRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.weather")))
...
func weatherBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WEATHER_DATA_TEST_WEATHER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WEATHER_DATA_TEST_WEATHER_ENTID"])
