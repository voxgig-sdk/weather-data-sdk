...
local WeatherDataSDK = {}
WeatherDataSDK.__index = WeatherDataSDK
...
WeatherDataSDK._make_feature = _make_feature


function WeatherDataSDK.new(options)
  local self = setmetatable({}, WeatherDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WeatherDataSDK:options_map()
...
function WeatherDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function WeatherDataSDK:get_root_ctx()
  return self._rootctx
end


function WeatherDataSDK:prepare(fetchargs)
...
function WeatherDataSDK:direct(fetchargs)
...
-- by passing it.
function WeatherDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WeatherDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WeatherDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WeatherDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WeatherDataSDK.new(sdkopts)
  sdk.mode = "test"
