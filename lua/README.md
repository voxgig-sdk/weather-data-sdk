...
The Lua SDK for the WeatherData API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:History()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/weather-data-sdk/releases)),
...
```
lua/
├── weather-data_sdk.lua    -- Main SDK module
...
The main module (`weather-data_sdk`) exports the SDK constructor
