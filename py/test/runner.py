...
class WeatherDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WeatherDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WeatherDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WeatherDataTestRunner.getenv("WEATHER_DATA_TEST_LIVE")
        override = WeatherDataTestRunner.getenv("WEATHER_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WeatherDataTestRunner.getenv(key)
        ...
        explain = WeatherDataTestRunner.getenv("WEATHER_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WEATHER_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WeatherDataTestRunner._test_control is not None:
            return WeatherDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WeatherDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WeatherDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WeatherDataTestRunner.load_test_control()
...
def load_env_local():
    WeatherDataTestRunner.load_env_local()


def env_override(m):
    return WeatherDataTestRunner.env_override(m)


def entity_data(v):
    return WeatherDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WeatherDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WeatherDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WeatherDataTestRunner.load_test_control()


def live_client_options():
    return WeatherDataTestRunner.live_client_options()


def live_delay_ms():
    return WeatherDataTestRunner.live_delay_ms()
