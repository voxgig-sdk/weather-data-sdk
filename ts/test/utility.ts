...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.WEATHERDATA_TEST_LIVE ||
    'TRUE' === process.env.WEATHERDATA_TEST_OVERRIDE
  ) {
  ...
  m.WEATHERDATA_TEST_EXPLAIN = process.env.WEATHERDATA_TEST_EXPLAIN || m.WEATHERDATA_TEST_EXPLAIN
