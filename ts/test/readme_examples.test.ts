...
//     (\`new WeatherDataSDK(...)\` / \`WeatherDataSDK.test(...)\` become
//     \`WeatherDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"history":{"test01":{"id":"test01"}},"weather":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WeatherDataSDK(...)\` and any
// \`WeatherDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WeatherDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WeatherDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
