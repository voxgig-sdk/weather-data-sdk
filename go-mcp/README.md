...
[MCP](https://modelcontextprotocol.io) server exposing the WeatherData SDK as
two agent tools — `weather-data_list` and `weather-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-data-mcp)
make build

# 2. Provide credentials via the environment
export WEATHER_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user weather-data \
  -- /absolute/path/to/weather-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./weather-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// weather-data_list: first page of records
{ "entity": "history" }
{ "entity": "history", "query": { } }

// weather-data_load: one record by id
{ "entity": "history", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user weather-data \
     -- "$PWD"/dist/*/weather-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `weather-data_list` and `weather-data_load` tools now appear
   in new sessions. Ask the agent to *"list history using weather-data"*
   and it calls `weather-data_list` with `{"entity":"history"}`.
...
```sh
export WEATHER_DATA_APIKEY=sk_live_xxx            # API key
export WEATHER_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `weather-data_list` tool
...
### Call the `weather-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `weather-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `weather-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEATHER_DATA_APIKEY` | API key sent with every request. |
| `WEATHER_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./weather-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather-data_load","arguments":{"entity":"history","query":{"id":1}}}}'
```
