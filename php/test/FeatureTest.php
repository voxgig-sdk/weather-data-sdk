...
class FtCtrl extends WeatherDataControl
{
...
class FtClient
{
    ...
    private WeatherDataUtility $utility;

    public function __construct(WeatherDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): WeatherDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeatherDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new WeatherDataUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WeatherDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeatherDataConfig::make_feature($name);
    ...
    private static function buildUrl(WeatherDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeatherDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeatherDataResponse($fetched);
        }
    ...
    private function fail(WeatherDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeatherDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
