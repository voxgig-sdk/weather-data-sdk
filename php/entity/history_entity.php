...
// WeatherData SDK History entity
...
class HistoryEntity
{
    ...
    {
        if ($args) {
            $this->_data = WeatherDataHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = WeatherDataHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws WeatherDataError on failure (item-5 convention).
     */
